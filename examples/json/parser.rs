...
pub fn json<'i, E: ParseError<&'i str> + ContextError<&'i str, &'static str>>(
  input: &'i str,
) -> IResult<&'i str, JsonValue, E> {
  delimited(ws, json_value, ws)(input)
}

/// `alt` is a combinator that tries multiple parsers one by one, until
/// one of them succeeds
fn json_value<'i, E: ParseError<&'i str> + ContextError<&'i str, &'static str>>(
  input: &'i str,
) -> IResult<&'i str, JsonValue, E> {
...
fn null<'i, E: ParseError<&'i str>>(input: &'i str) -> IResult<&'i str, &'i str, E> {
...
/// success.
fn boolean<'i, E: ParseError<&'i str>>(input: &'i str) -> IResult<&'i str, bool, E> {
...
fn string<'i, E: ParseError<&'i str> + ContextError<&'i str, &'static str>>(
  input: &'i str,
) -> IResult<&'i str, String, E> {
  preceded(
    '\"',
...
/// like escaping
fn character<'i, E: ParseError<&'i str>>(input: &'i str) -> IResult<&'i str, char, E> {
...
fn unicode_escape<'i, E: ParseError<&'i str>>(input: &'i str) -> IResult<&'i str, char, E> {
  alt((
    // Not a surrogate
    u16_hex
...
fn u16_hex<'i, E: ParseError<&'i str>>(input: &'i str) -> IResult<&'i str, u16, E> {
  take(4usize)
...
fn array<'i, E: ParseError<&'i str> + ContextError<&'i str, &'static str>>(
  input: &'i str,
) -> IResult<&'i str, Vec<JsonValue>, E> {
  preceded(
    ('[', ws),
    cut(terminated(
...
fn object<'i, E: ParseError<&'i str> + ContextError<&'i str, &'static str>>(
  input: &'i str,
) -> IResult<&'i str, HashMap<String, JsonValue>, E> {
  preceded(
    ('{', ws),
    cut(terminated(
...
fn key_value<'i, E: ParseError<&'i str> + ContextError<&'i str, &'static str>>(
  input: &'i str,
) -> IResult<&'i str, (String, JsonValue), E> {
